...
export const ContinueMemorizationCard = () => {
  ...
  // Helper: check if a surah is fully memorized
  ...
  // Find latest block whose surah is NOT yet complete
  ...
  const surah = activeBlock
    ? chapters?.find(c => c.id === activeBlock.surah_id)
    : null;

  // Calculate progress for the active surah
  ...
  // Show "Start Memorizing" when no in-progress surah exists
  if (!activeBlock) {
    return (
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-stone-300">
                Keep going—Allah sees your effort.
              </p>
            </div>
          </div>
  ...
  const handleContinue = () => {
    navigate('/memorize', {
      state: {
        surahId: activeBlock.surah_id,
        nextAyahStart: nextAyah,
      },
    });
  };
