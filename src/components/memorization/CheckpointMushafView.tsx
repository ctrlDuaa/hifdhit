...
/**
 * Consolidation checkpoint view: renders the full Mushaf page(s) covering the
 * newly-memorized range. Words within the range are blurred (hover to reveal),
 * surrounding context is shown normally. If the range spans multiple pages, the
 * user can flip page-by-page. Surah headers + Bismillah render like a real mushaf.
 */
...
export const CheckpointMushafView = ({ surahId, startAyah, endAyah, className }: Props) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('CheckpointMushafView load failed:', e);
  ...
  const isInRange = (verseKey: string | undefined): boolean => {
    if (!verseKey) return false;
    const [s, a] = verseKey.split(':').map(Number);
    if (s !== surahId) return false;
    if (typeof a !== 'number') return false;
    return a >= startAyah && a <= endAyah;
  };
              ...
              <div key={`p${pageNum}-line-${ln}-wrap`}>
                ...
                >
                  {words.map((word, wi) => {
                    ...
                    const inRange = isInRange(word.verse_key);
